...
export default class Town {
  ...
  async addPlayer(userName: string, socket: CoveyTownSocket): Promise<Player> {
    ...
    socket.on('chatMessage', (message: ChatMessage) => {
      for (const player of this._players) {
        if (player.location.interactableID === message.interactableId) {
    ...
    socket.on('interactableUpdate', (update: Interactable) => {
      const updatedInteractable = this._interactables.find(
        interactable => interactable.id === update.id,
      );
      if (updatedInteractable instanceof PosterSessionArea) {
        updatedInteractable.updateModel(update as PosterSessionArea);
      } else if (updatedInteractable instanceof ViewingArea) {
        updatedInteractable.updateModel(update as ViewingArea);
      }
  ...
  public addPosterSessionArea(posterSessionArea: PosterSessionAreaModel): boolean {
    ...
    if (!area || area.title || !posterSessionArea.imageContents || !posterSessionArea.title) {
      return false;
    }
